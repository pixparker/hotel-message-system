...
**Scope**: Drizzle migration that enables RLS on every tenant table and adds `USING (organization_id = current_setting('app.current_org')::uuid)` policies. Add `withTenant` middleware in api that runs `SET LOCAL app.current_org` per request after auth.
